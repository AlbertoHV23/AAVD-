...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrev, PSTR cmd, int show)
{
	...
	//MessageBox(0, _folder, fold, MB_ABORTRETRYIGNORE); //
	_hInst = hInst;
	_show = show;
...
BOOL CALLBACK ProcDialogo1(HWND Dlg, UINT Mensaje, WPARAM wParam, LPARAM lparam)
{
	...
	case WM_COMMAND:
	{
		...
		case ID_CITAS_ELIMINARCITA40012:
		{
			DialogBox(_hInst, MAKEINTRESOURCE(IDD_DIALOGO6), Dlg, ProcDialogo6);
			return true;
		}
...
BOOL CALLBACK ProcDialogo2(HWND Dlg, UINT Mensaje, WPARAM wParam, LPARAM lparam)
{
	...
	case WM_COMMAND:
	{
...

		{
			...
			SendDlgItemMessage(Dlg, IDC_Hora, WM_GETTEXT, WPARAM(11), (LPARAM)AUX.Hora);
...
//case IDC_EDIT1:
//	SendDlgItemMessage(Dlg, IDC_EDIT1, WM_GETTEXT, (WPARAM)80, (LPARAM)dr.NameDoc);
////
//
//case IDC_EDIT2:
//	SendDlgItemMessage(Dlg, IDC_EDIT2, WM_GETTEXT, (WPARAM)80, (LPARAM)dr.Numero);
			//
...
void llenalista(HWND Dlg) {
	...
	//strcat(MostrarDatos, datos.c_str());
		...
		SendDlgItemMessage(Dlg, IDC_LIST1, LB_ADDSTRING, 0, (LPARAM)aux->Costo);

	
	
		SendDlgItemMessage(Dlg, IDC_LIST1, LB_SETITEMDATA, k, k);
		k = +1;
		aux = aux->sig;
...
void GuardariNFO() {
	...
	ofstream archivaldo2;
	archivaldo2.open(file2, ios::binary | ios::trunc);
	if (archivaldo2.is_open())
	{
		// LEER la lista ligada
		aux = INFO;
		while (aux != 0)
		{
			archivaldo2.write((char*)aux, sizeof(INFO));
